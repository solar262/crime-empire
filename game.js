...
const icons = {
  cornerShop:'🏪', nightClub:'🎧', autoGarage:'🏎️', privateSecurity:'🛡️', importWarehouse:'📦', luxuryHotel:'🏨', mediaCompany:'📺', tower:'🏙️',
  lookout:'👀', driver:'🏁', fixer:'🧰', lieutenant:'🎖️', strategist:'♟️', captain:'🧑‍✈️', negotiator:'🤝', mastermind:'🧠',
  street:'💵', club:'🎭', garage:'🔧', dock:'⚓', casino:'🎲', summit:'👑',
  office:'🏢', training:'🥊', logistics:'🚚', legal:'⚖️'
};
...
function claimDaily(){
  const t = todayKey();
  if(state.lastDaily === t) return showModal('Daily Reward', '<div class="reward-box"><strong>Already claimed today.</strong><p>Come back tomorrow.</p></div>');
...
function tutorial(){ state.tutorialSeen = true; save(false); showModal('How to Play', '<ol><li><strong>Collect Tribute</strong> to get starting cash.</li><li><strong>Buy Empire businesses</strong> to earn every second.</li><li><strong>Hire Crew</strong> to raise power.</li><li><strong>Run Jobs</strong> for big payouts.</li><li><strong>Take Map districts</strong> to control the city.</li><li><strong>Upgrade HQ</strong> for permanent boosts.</li></ol>'); }

function cardIcon(id){ return `<div class="item-icon">${icons[id] || '⭐'}</div>`; }
...
function objective(){ if(state.cash < 50) return 'Collect your first $50'; if(totalFronts() < 1) return 'Buy your first business'; if(totalCrew() < 3) return 'Hire 3 crew members'; if(state.respect < 50) return 'Run jobs for respect'; if((state.hq.office || 0) < 2) return 'Upgrade Back Office'; if(state.districts.length < 1) return 'Take your first district'; if(rawIncome() < 100) return 'Reach $100/sec income'; return 'Take over the city'; }
function render(){
  ...
  const db=document.getElementById('dailyButton'); db.textContent = state.lastDaily === todayKey() ? '✅ Daily' : '🎁 Daily'; db.disabled = state.lastDaily === todayKey();
  const bb=document.getElementById('boostButton'); if(Date.now()<state.boostUntil){bb.textContent=`⚡ ${Math.ceil((state.boostUntil-Date.now())/1000)}s`;bb.disabled=true}else{bb.textContent='⚡ 2x Boost';bb.disabled=false}
  renderList('businesses', businesses, 'business'); renderList('crew', crew, 'crew'); renderJobs(); renderHq(); renderDistricts();
}
