...
const missions = [
  ...
  { text: 'Upgrade any HQ room to level 3', goal: s => Math.min(1, Math.max(...Object.values(s.hq)) / 3), done: s => Math.max(...Object.values(s.hq)) >= 3, reward: 3200 },
...
function tutorial(){ state.tutorialSeen = true; save(false); showModal('How to Play', '<ol><li>Collect tribute for starting cash.</li><li>Buy Empire businesses for automatic income.</li><li>Hire Crew to increase Power.</li><li>Complete Missions to earn chest points.</li><li>Open Chests for big rewards.</li><li>Take Map districts and upgrade HQ.</li></ol>'); }
...
function render(){
  ...
  renderMission(); renderList('businesses', businesses, 'business'); renderList('crew', crew, 'crew'); renderJobs(); renderHq(); renderDistricts();
}
...
document.getElementById('fightButton').onclick=fight; document.getElementById('boostButton').onclick=activateBoost; document.getElementById('layLowButton').onclick=layLow; document.getElementById('saveButton').onclick=()=>save(true); document.getElementById('dailyButton').onclick=claimDaily; document.getElementById('tutorialButton').onclick=tutorial; document.getElementById('missionButton').onclick=claimMission; document.getElementById('chestButton').onclick=openChest; document.getElementById('closeModal').onclick=closeModal; document.getElementById('modal').onclick=e=>{ if(e.target.id==='modal') closeModal(); };
